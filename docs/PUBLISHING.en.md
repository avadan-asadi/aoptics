...
This guide covers everything needed to build **aoptics**, upload it to PyPI, and
...
The Python package importable as `import aoptics` is distributed on PyPI under the name
**`aoptics`** (the plain name `aoptics` was already taken by an unrelated project).
...
3. Before your first real upload, **edit `pyproject.toml`**: replace every `OWNER` in the
   ...
   `authors` field if you want a named maintainer instead of "aoptics Contributors".
...
```bash
cd aoptics/          # the folder containing pyproject.toml
...
the `aoptics` project (after the very first upload, which needs an account-wide
token; you can narrow the scope afterwards).

After this first manual upload, `pip install aoptics` works for everyone.
...
1. Go to your project on PyPI: `https://pypi.org/manage/project/aoptics/settings/publishing/`
...
2. Under "Trusted Publishers", click "Add a new publisher" and fill in:
   ...
   - Repository name: your repo name (e.g. `aoptics`)
...
1. Bump the version number in **`aoptics/__init__.py`** (`__version__ = "2.2.0"`, etc.) --
...
- [ ] `aoptics/__init__.py`: `__version__` bumped.
...
- [ ] `pip install --upgrade aoptics` and confirm the new version.
